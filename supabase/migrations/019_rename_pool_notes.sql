-- ============================================
-- Minerva v2 — 019: Rename pool notes to comments
-- ============================================

-- 1. Rename the column in the table
ALTER TABLE public.pool_rules RENAME COLUMN notes TO comments;

-- 2. Drop the old functions that use 'p_notes'
DROP FUNCTION IF EXISTS public.create_pool_rule(TEXT, TEXT, SMALLINT[], JSONB, TEXT[], TEXT[], BOOLEAN, BOOLEAN, TEXT);
DROP FUNCTION IF EXISTS public.update_my_pool_rule(UUID, TEXT, TEXT, SMALLINT[], JSONB, TEXT[], TEXT[], BOOLEAN, BOOLEAN, TEXT);
DROP FUNCTION IF EXISTS public.get_my_pool_rules();

-- 3. Recreate the functions using 'p_comments' and fetching 'comments'
