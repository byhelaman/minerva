-- ============================================
-- Minerva v2 — 022: Chat RAG Functions
-- ============================================
...
--
-- Depends on: 006_schedules_realtime.sql, 008_schedules_optimization.sql
...
-- Fast date-range + branch/program filtering (used by all RPCs)
CREATE INDEX IF NOT EXISTS idx_schedule_entries_date_branch_program
  ON public.schedule_entries (date, branch, program);

-- Covering index: avoids heap lookups for the common SELECT column set
...
BEGIN
  ...
  -- Validate range (max 90 days) with safe date casting
  BEGIN
  ...
  -- Collect matched instructor names
...
-- Drop old signature (without p_threshold) to avoid ambiguity
DROP FUNCTION IF EXISTS public.chat_get_stats(TEXT, TEXT, TEXT, TEXT);

CREATE OR REPLACE FUNCTION public.chat_get_stats(
  p_start_date  TEXT,
  p_end_date    TEXT,
  p_group_by    TEXT  DEFAULT 'instructor',   -- 'instructor' | 'date' | 'branch'
...
-- Handles end_time = '' (entries without end_time are skipped in overlap check).
...
BEGIN
  ...
  IF p_instructor_list IS NOT NULL THEN
    -- List provided: check ALL of them — available = no conflicting entry
  ...
  ELSE
    -- No list: instructors with classes that day minus those with conflicts
