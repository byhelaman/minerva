-- =============================================
-- Migration 015: fix language matching with unaccent
-- =============================================
-- Fixes accent-insensitive language matching across all chat RPCs.
-- "portugues" now matches "Português", "aleman" matches "Alemán", etc.
      ...
      (SELECT json_agg(
       ...
       WHERE ip.can_evaluate = true
         ...
         AND (p_language IS NULL OR EXISTS (
           ...
           WHERE unaccent(lower(l)) = unaccent(lower(p_language))
         ))
         AND EXISTS (
           SELECT 1
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'total_languages', COUNT(DISTINCT unaccent(lower(lang))),
    'languages', COALESCE(
      json_agg(
        json_build_object(
          'language', lang,
