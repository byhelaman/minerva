...
Todas las funciones se ejecutan en **Deno** y se despliegan con `--no-verify-jwt` — la autenticación se maneja internamente con funciones propias.
...
| Lote con `batch: true` | Procesa array de requests en paralelo via `Promise.allSettled` |
...
POST → zoom-sync
     │
     ├── Obtiene todos los usuarios de Zoom (page_size=300)
...
**Autenticación:** Verificación de firma HMAC via headers `x-zm-signature` + `x-zm-request-timestamp` con `ZOOM_WEBHOOK_SECRET`. Timestamp debe estar dentro de ventana de 5 minutos.
...
| `append-rows` | Agrega filas al final de una tabla |
...
|-------------|-------------|
| `verifyPermission(req, supabase, permission)` | **Función principal de auth.** Extrae Bearer token, obtiene usuario, crea cliente Supabase del usuario para llamar RPC `has_permission`. Acepta string (match exacto) o array (lógica OR). Retorna el objeto usuario. |
...
| `verifyInternalKey(req)` | Verifica header `x-internal-key` contra `INTERNAL_API_KEY` env var. Para llamadas server-to-server. |
...
| `delete_role(name)` | nivel ≥ 100 | Elimina rol (protege roles de sistema) |
...
| Función | Propósito |
|---------|-----------|
| `store_zoom_credentials(...)` | Elimina secretos antiguos de Vault, crea nuevos, actualiza `zoom_account`. Atómico. |
| `delete_zoom_secrets(ids[])` | Elimina secretos de Vault por array de IDs |
| `store_microsoft_credentials(...)` | Mismo patrón. Preserva config de OneDrive al renovar tokens del mismo usuario. |
| `delete_microsoft_secrets(ids[])` | Mismo patrón para Microsoft |
| `update_microsoft_config(type, id, name, ...)` | Actualiza columnas de config de carpeta o archivo en `microsoft_account` |
| `create_oauth_state(user_id)` | Genera estado hex de 32 bytes, limpia expirados, inserta con expiración de 10 min |
| `validate_oauth_state(state)` | Busca estado, retorna `user_id` si válido, elimina estado (uso único) |

### 3.4 Migraciones

| Migración | Contenido |
|-----------|-----------|
| `001_core_access` | Tablas `roles`, `permissions`, `role_permissions`, `profiles`. Seeds de roles y permisos. Hook JWT, triggers de perfil |
| `002_user_management` | Funciones RPC para gestión de usuarios y roles (`get_all_users`, `update_user_role`, `delete_user`, etc.) |
| `003_zoom_integration` | Tablas `zoom_account`, `oauth_states`, `zoom_users`, `zoom_meetings`. Vista `zoom_credentials_decrypted`. RPCs de credenciales |
| `004_webhooks_bug_reports` | Tablas `webhook_events`, `bug_reports`. Funciones de limpieza y reuniones activas |
| `005_realtime_security` | `REPLICA IDENTITY FULL` en `profiles` para eventos Realtime |
| `006_microsoft_integration` | Tabla `microsoft_account`. Vista `microsoft_credentials_decrypted`. RPCs de credenciales y config |
| `008_published_schedules` | Tabla `published_schedules`. Políticas RLS. Habilitación Realtime |
| `009_schedule_entries` | Tabla `schedule_entries` con unique constraint compuesto. Políticas RLS. Habilitación Realtime |
| `012` | RPC `delete_zoom_secrets` |
| `013` | RPC `verify_user_password` |
| `014` | Permiso `reports.manage` + asignación a admin/super_admin |
| `015` | RPC `delete_microsoft_secrets` |
| `016` | Recrea vista Microsoft con columna `microsoft_name` |
...
| `oauth_states` | `idx_oauth_states_expires` | B-tree |
| `bug_reports` | `idx_bug_reports_user_id`, `_status`, `_created_at` | B-tree |
| `webhook_events` | `idx_webhook_events_event_type`, `_created_at` | B-tree |
| `published_schedules` | `idx_published_schedules_date`, `_created` | B-tree |
| `schedule_entries` | `idx_schedule_entries_date`, `_instructor`, `_sync` (parcial: WHERE synced_at IS NULL) | B-tree |
...
2. **Verificaciones en RPCs** — `update_user_role`, `delete_user`, `create_role` verifican jerarquía independientemente
3. **Roles de sistema protegidos** — `super_admin`, `admin`, `operator`, `viewer` no se pueden eliminar
...
|-------|-----------|-------------|
| `profiles` | `REPLICA IDENTITY FULL` (migración 005) | Manual en Dashboard |
| `published_schedules` | Publicación `supabase_realtime` (migración 008) | Automática |
| `schedule_entries` | Publicación `supabase_realtime` (migración 009) | Automática |
...
Todas las funciones listadas se configuran con `verify_jwt = false`:

| Función | Notas |
|---------|-------|
| `zoom-api` | Configuración estándar |
| `zoom-auth` | Configuración estándar |
| `zoom-sync` | Import map personalizado (`deno.json`) |
| `zoom-webhook` | Configuración estándar |

Las funciones de Microsoft (`microsoft-auth`, `microsoft-graph`) no están listadas explícitamente — usan configuración por defecto.
...
| `ZOOM_WEBHOOK_SECRET` | zoom-webhook | Verificación HMAC de webhooks |
| `ZOOM_WHITELIST_EMAILS` | zoom-sync | Emails a incluir siempre en sync |
| `INTERNAL_API_KEY` | zoom-sync | Clave para llamadas server-to-server (cronjobs) |

> **Nota:** Las credenciales de Zoom (`ZOOM_CLIENT_ID`, `ZOOM_CLIENT_SECRET`, etc.) se leen implícitamente del entorno por el flujo OAuth pero no están documentadas en config.toml.
